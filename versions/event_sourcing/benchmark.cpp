...
std::string generateReport(const std::string& version_name, size_t num_orders, 
    ...
    report << "- **Implementation**: Event Sourcing\n";
    report << "- **Performance Target**: ~300K orders/sec, ~3μs\n";
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "========================================\n";
    std::cout << "Event Sourcing Version Performance Benchmark\n";
    std::cout << "========================================\n";
    ...
    for (size_t i = 0; i < warmup; ++i) {
        try {
            ...
            engine.process_order_es(order_copy.release());
        } catch (...) {
    ...
    for (size_t i = warmup; i < num_orders; ++i) {
        try {
            ...
            auto trades = engine.process_order_es(order_copy.release());
    ...
    std::string report = generateReport("Event Sourcing", num_orders - warmup,
