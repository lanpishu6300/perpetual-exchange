...
bool ProductionMatchingEngine::initialize(const std::string& config_file) {
    ...
    // Initialize logger
    std::string log_file = config.getString("log.file", "");
    std::string log_level_str = config.getString("log.level", "INFO");
    LogLevel log_level = LogLevel::INFO;
    if (log_level_str == "DEBUG") log_level = LogLevel::DEBUG;
    else if (log_level_str == "WARN") log_level = LogLevel::WARNING;
    ...
    if (config.getBool("persistence.enabled", true)) {
        std::string db_path = config.getString("persistence.db_path", "./data");
