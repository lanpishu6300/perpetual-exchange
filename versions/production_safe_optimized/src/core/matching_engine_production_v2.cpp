...
ProductionMatchingEngineV2::ProductionMatchingEngineV2(InstrumentID instrument_id)
    ...
    , persistence_queue_(100000) {  // 100K queue size (increased for high throughput)
...
bool ProductionMatchingEngineV2::initialize(const std::string& config_file) {
    ...
    std::string log_level_str = config.getString("log.level", "WARN");
    LogLevel log_level = LogLevel::WARNING;  // Default to WARN for performance
    ...
    // Check both performance.async_persistence AND persistence.enable
    // If either is false, disable persistence
    bool persistence_enabled = config.getBool("persistence.enable", true);
    if (!enable_async_persistence_ || !persistence_enabled) {
        enable_async_persistence_ = false;
        LOG_INFO("Persistence disabled (performance.async_persistence=" + 
                 std::to_string(enable_async_persistence_) + 
                 ", persistence.enable=" + std::to_string(persistence_enabled) + ")");
    }
    
    if (enable_async_persistence_) {
...
bool ProductionMatchingEngineV2::checkRateLimitCached(UserID user_id) {
    ...
    if (!global_rate_limiter_->checkLimit()) {
        return false;
    }
    
    if (!user_rate_limiter_->checkLimit(user_id)) {
        return false;
    }
...
void ProductionMatchingEngineV2::enqueuePersistence(const Order& order, const std::vector<Trade>& trades) {
    ...
    if (!persistence_queue_.push(task)) {
        ...
        LOG_WARNING("Persistence queue full, dropping task");
    }
}
...
void ProductionMatchingEngineV2::persistenceWorker() {
    ...
    while (persistence_running_.load(std::memory_order_relaxed) || persistence_queue_.size() > 0) {
        ...
        if (persistence_queue_.pop(task)) {
            ...
            // Queue empty, use yield for lower latency (instead of sleep)
