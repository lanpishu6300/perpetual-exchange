...
DistributedEventStore::DistributedEventStore(const DistributedEventStoreConfig& config)
    : config_(config) {
    // If single node mode, disable consensus and replication
    if (config_.single_node_mode) {
        config_.enable_consensus = false;
        config_.enable_replication = false;
        config_.replication_factor = 1;
    }
    
    // Initialize node availability (only if not in single node mode)
    if (!config_.single_node_mode && config_.enable_replication) {
...
bool DistributedEventStore::initialize(const std::string& data_dir) {
    ...
    // Start replication thread only if replication is enabled
    if (config_.enable_replication && !config_.single_node_mode) {
...
bool DistributedEventStore::append_event(const Event& event) {
    ...
    // Queue for replication only if replication is enabled
    if (config_.enable_replication && !config_.single_node_mode) {
...
std::vector<Event> DistributedEventStore::get_events(SequenceID from, SequenceID to) const {
    ...
    // If consensus enabled and not in single node mode, verify with other nodes
    if (config_.enable_consensus && !config_.single_node_mode && events.size() > 0) {
        // In production, would query multiple nodes and compare
        // For now, just return local events
        // TODO: Implement actual consensus verification
    }
...
bool DistributedEventStore::replicate_event(const Event& event, 
    ...
    // In single node mode, replication is disabled
    if (config_.single_node_mode || !config_.enable_replication) {
        return true;  // Success (no-op in single node mode)
    }
...
SequenceID DistributedEventStore::get_consensus_sequence() const {
    // In single node mode, just return local sequence
    if (config_.single_node_mode || !config_.enable_consensus) {
    ...
    // TODO: Implement actual consensus query
    return local_store_->get_latest_sequence();
}
...
Command Command::deserialize(const std::string& data) {
    ...
    if (!std::getline(iss, token, ',')) return cmd;
    cmd.type = static_cast<Type>(safe_stoul(token));
    if (!std::getline(iss, token, ',')) return cmd;
    cmd.order_id = safe_stoull(token);
    if (!std::getline(iss, token, ',')) return cmd;
    cmd.user_id = safe_stoull(token);
    if (!std::getline(iss, token, ',')) return cmd;
    cmd.instrument_id = safe_stoul(token);
    if (!std::getline(iss, token, ',')) return cmd;
    cmd.side = static_cast<OrderSide>(safe_stoul(token));
    if (!std::getline(iss, token, ',')) return cmd;
    cmd.price = safe_stoll(token);
    if (!std::getline(iss, token, ',')) return cmd;
    cmd.quantity = safe_stoll(token);
    if (!std::getline(iss, token, ',')) return cmd;
    cmd.order_type = static_cast<OrderType>(safe_stoul(token));
...
VersionedEvent VersionedEvent::deserialize(const std::string& data, EventVersion version) {
    ...
    if (!std::getline(iss, token, ',')) return ve;
    ve.version = safe_stoul(token);
    if (!std::getline(iss, token, ',')) return ve;
    ve.min_supported_version = safe_stoul(token);
    if (!std::getline(iss, token, ',')) return ve;
    ve.max_supported_version = safe_stoul(token);
