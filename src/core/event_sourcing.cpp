...
Event Event::deserialize(const std::string& data_str) {
    ...
    switch (event.type) {
        ...
        case EventType::TRADE_EXECUTED:
            if (!std::getline(iss, token, ',')) break;
            event.data.trade_executed.trade.buy_order_id = safe_stoull(token);
            if (!std::getline(iss, token, ',')) break;
            event.data.trade_executed.trade.sell_order_id = safe_stoull(token);
            if (!std::getline(iss, token, ',')) break;
            event.data.trade_executed.trade.buy_user_id = safe_stoull(token);
            if (!std::getline(iss, token, ',')) break;
            event.data.trade_executed.trade.sell_user_id = safe_stoull(token);
            if (!std::getline(iss, token, ',')) break;
            event.data.trade_executed.trade.instrument_id = safe_stoul(token);
            if (!std::getline(iss, token, ',')) break;
            event.data.trade_executed.trade.price = safe_stoll(token);
            if (!std::getline(iss, token, ',')) break;
            event.data.trade_executed.trade.quantity = safe_stoll(token);
            if (!std::getline(iss, token, ',')) break;
            event.data.trade_executed.trade.timestamp = safe_stoll(token);
            if (!std::getline(iss, token, ',')) break;
            event.data.trade_executed.trade.sequence_id = safe_stoull(token);
            if (!std::getline(iss, token, ',')) break;
            event.data.trade_executed.trade.is_taker_buy = (safe_stoul(token) == 1);
            break;
        default:
            break;
    }
