...
struct DistributedEventStoreConfig {
    ...
    bool enable_consensus = false;     // Enable consensus protocol (default: false for local testing)
    bool enable_replication = false;   // Enable replication (default: false for local testing)
    
    // Single node mode (for local testing)
    // When true, consensus and replication are disabled
    bool single_node_mode = true;      // Default: single node mode for local testing
    
    // Constructor for easy configuration
    DistributedEventStoreConfig() = default;
    ...
    static DistributedEventStoreConfig create_distributed(NodeID node_id, 
        ...
        config.replica_nodes = replica_nodes;
