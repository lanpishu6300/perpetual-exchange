...
// 安全性能测试 - 使用栈上Order对象，避免内存问题
int main(int argc, char* argv[]) {
    int num_threads = 2;
    int duration_seconds = 5;
    int total_orders = 5000;
    ...
    std::cout << "========================================" << std::endl;
    std::cout << "Safe Performance Test: MatchingEngineOptimizedV3" << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    std::string event_dir = "./test_v3_safe_event_" + std::to_string(timestamp);
    std::string persist_dir = "./test_v3_safe_persist_" + std::to_string(timestamp);
    ...
    latencies.reserve(total_orders / 10);
        ...
        threads.emplace_back([&, t]() {
            ...
            while (orders_processed < orders_per_thread && 
                ...
                if (orders_processed % 1000 == 0) {
                    std::cout << "Thread " << t << ": processed " << orders_processed << " orders" << std::endl;
                }
            }
        });
    }
    ...
    std::cout << "  Min: " << std::fixed << std::setprecision(2) << (min_latency / 1000.0) << " μs" << std::endl;
